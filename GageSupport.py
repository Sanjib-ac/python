...
def SaveVoltageFile(filename, buffer, stHeader):
    try:
        ...
        try:
            ...
            data = map(lambda x: (((stHeader['SampleOffset'] - x) / stHeader['SampleRes']) * scale_factor) + offset,
                       buffer.tolist())
...
def SaveFile(filename, channel, buffer, format, stHeader):
    if format == TYPE_SIG:
        ...
        return SaveBinaryFile(filename, buffer,
                              stHeader['SampleBits'])  # straight binary, might need header for sample bits
